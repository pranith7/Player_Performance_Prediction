...
for file in os.listdir(input_folder):
    ...
    if file.endswith(".csv"):
        ...
        total_runs = pd.DataFrame(data.groupby(['batting_team', 'bowling_team', 'match_id','start_date', 'striker'])['runs_off_bat'].sum())
        total_runs = total_runs.rename(columns={"runs_off_bat": "Batsman_runs"})

        # Number of sixes
        sixes = pd.DataFrame(data[data['runs_off_bat'] == 6].groupby(['batting_team', 'bowling_team', 'match_id','start_date', 'striker'])['runs_off_bat'].count())
        sixes = sixes.rename(columns={"runs_off_bat": "6"})

        # Number of fours
        fours = pd.DataFrame(data[data['runs_off_bat'] == 4].groupby(['batting_team', 'bowling_team', 'match_id','start_date', 'striker'])['runs_off_bat'].count())
        fours = fours.rename(columns={"runs_off_bat": "4"})

        # Number of balls
        balls_faced = pd.DataFrame(data.groupby(['batting_team', 'bowling_team', 'match_id','start_date', 'striker'])['ball'].nunique())
        balls_faced = balls_faced.rename(columns={"ball": "total_legal_balls_faced"})
